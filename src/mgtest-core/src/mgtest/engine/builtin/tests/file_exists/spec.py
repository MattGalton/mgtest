...
class FileExists(TestSpec):
    ...
    timeout: float | None = Field(
        default=None, gt=0, description="Maximum seconds to wait; omit to check once."
    )
    interval: float = Field(
        default=0.1, gt=0, description="Seconds between existence checks while waiting."
    )

    class Output(BaseModel):
        exists: bool = Field(
            default=False, description="Whether the path existed when the check completed."
        )
