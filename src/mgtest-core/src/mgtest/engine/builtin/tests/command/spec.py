...
class Command(TestSpec):
    ...
    shell_command: str | None = Field(
        default=None, description="Shell command to run; specify this or python_command."
    )
    python_command: str | None = Field(
        default=None, description="Python code to run; specify this or shell_command."
    )
    cwd: Path | None = Field(default=None, description="Working directory for the command.")
    environment: dict[str, str] = Field(
        default_factory=dict, description="Environment variables added to the command process."
    )
    timeout: float = Field(default=30.0, gt=0, description="Maximum command runtime in seconds.")
    expected_exit_code: int = Field(
        default=0, description="Exit status required for the check to pass."
    )
    stdout_contains: str | None = Field(
        default=None, description="Text that must occur in standard output."
    )
    stderr_contains: str | None = Field(
        default=None, description="Text that must occur in standard error."
    )
