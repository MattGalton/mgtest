...
class HttpRequest(TestSpec):
    ...
    expected_status: int | list[int] = Field(default=200, description="Accepted HTTP status code or codes.")
    body_contains: str | None = Field(default=None, description="Text that must occur in the response body.")
    json_equals: Any | None = Field(default=None, description="JSON value required for the response body.")
    timeout: float = Field(default=10.0, gt=0, description="Maximum seconds to wait for a passing response.")
    interval: float = Field(default=0.1, gt=0, description="Seconds between requests while waiting.")
    ...
    class Output(BaseModel):
        ...
        json_data: Any | None = Field(default=None, description="Decoded JSON response body, if present.")
