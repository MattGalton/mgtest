...
def write_jetbrains_schemas(src_schemas_dir: Path, idea_dir: Path):
    ...
    Args:
        src_schemas_dir: Path where mgtest schemas (test.json, resource.json) are stored.
    ...
    # Create an ORM of the XML file that tells your jetbrains IDE to link t_* and r_* to our schemas
    relative_schemas = (
        lambda f: f"$PROJECT_DIR$/{tgt_schemas_dir.relative_to(idea_dir.parent).as_posix()}/{f}"
    )
    app = _Application(
        component=_ApplicationComponent(
            ...
            yaml_schema_mappings=_YAMLSchemaMappings(
                mapping=[
                    _SchemaMapping(file=relative_schemas("test.json"), pattern="**/t_*.yml"),
                    _SchemaMapping(file=relative_schemas("resource.json"), pattern="**/r_*.yml"),
                ]
            ),
        )
    )
